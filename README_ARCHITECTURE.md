...
2. 드라이버/통신  sllidar_ros2 (USB) / GPS 브릿지(MQTT)
                        ▲
1. 하드웨어       RPLIDAR C1 / Raspberry Pi 5 / 전원부·GPS모듈
```
...
```
        ...
        │              │
        ├──────────────┤
        │ Raspberry Pi5 │  ← 무게 중심을 낮추기 위해 하단 배치
        │ + 보조배터리   │
        └──────────────┘
              │
        백팩 측면 포켓에 수직으로 삽입
...
| Raspberry Pi 5 | 중~하단 | 발열 방출 공간 확보, 무게중심 하향 |
| 보조배터리 | 최하단 | 가장 무거운 부품 → 무게중심 안정화 |
| USB 케이블(라이다-Pi) | 내부 관통홀 | 외부 노출 시 걸림/단선 위험 |
...
- **발열**: Pi5 + SLAM 연산 시 발열 있음 → 방열판/액티브 쿨러 필수, 밀폐 케이스 지양
...
Pi5+실제 센서가 오면 launch 인자 하나(`use_mock:=false`)로 전환한다.
...
6. (Pi5 도착 시) 워크스페이스 통째로 이식, `raspi-config`/GPIO 관련 항목만 추가 설정

### 5.3 개발 단계 (Phase) — 2026-09-08 정의
Pi5가 없는 지금과, Pi5로 실제 휴대 이동하는 이후는 목적 자체가 달라서 단계를 나눈다.

| | **Phase 1 (현재)** | **Phase 2 (Pi5 도착 후)** |
|---|---|---|
