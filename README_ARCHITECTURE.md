...
### 3.5.9 앱 네비게이션 구조 — 하단 탭 4개 (2026-09-17 확정)
IoT 동반 앱의 일반적인 패턴(iRobot Home: Home/Clean/History/Settings 등)을
참고해 확정. 연결 화면(rosbridge 주소 입력)은 탭이 아니라 **연결 전 게이트
화면**이고, 연결 성공 시 아래 4탭 메인 화면으로 전환된다(`go_router` 사용).

| 탭 | 내용 | 대응 Tier 항목 |
|---|---|---|
| **홈(진단)** | 라이다/IMU/Orange Pi/배터리 상태 체크 | Tier1-4 |
| **트래킹** | 실시간 스캔 레이더+누적지도(데모: [artifact](https://claude.ai/code/artifact/596148a7-35da-4410-bd0f-bc0dce5fa559)) + 시작/정지 버튼 + 저장 | Tier1-6 + Tier1-3 |
| **목록** | 저장된 세션 목록 — 세션명, 완성도%(2.6번 `completeness_note`), 좌표 입력란, 상단에 저장공간 여유 표시 | Tier2-7,8 + 2.5/2.6번 스키마 |
| **설정** | 연결 주소 변경, 환경 프리셋+필터 세부조절, **가이드(용어/옵션 설명집)** | Tier1-2,7 + 신규 |

**좌표 입력 — GPS 자동 획득으로 확정**: 사용자가 직접 좌표를 타이핑하지 않고,
"목록" 화면에서 📍 버튼을 누르면 `geolocator` 패키지로 위치 권한 요청 후
현재 위치를 자동으로 채운다. 좌표만으론 사람이 알아보기 어려우니 **OSM
Nominatim(무료 역지오코딩)**으로 대략적인 지명을 자동 제안하고 사용자가
수정 가능하게 한다(`flutter_map`과 같은 OSM 생태계라 궁합 좋음). 실내라 GPS
신호가 없으면 좌표 없이 이름만 입력하는 것으로 자연 폴백.

**설정 > 가이드 메뉴(신규)**: 라이다/센서 옵션값의 의미와 조정 방법을 앱 안에
용어집처럼 담아둔다 — 예: Hz(스캔 주파수) 높이면 회전은 빨라지지만 샘플레이트를
같이 안 올리면 각분해능이 떨어짐, 노이즈 필터 임계값의 의미, IMU/EKF 센서퓨전
개념 등. 매번 문서를 찾아보지 않고 앱 안에서 바로 확인 가능하게.
